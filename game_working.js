...
function shootBullet() {
    ...
    if (currentTime - gameState.lastShotTime > gameState.shotCooldown) {
        // プレイヤーの向いている方向に弾を発射
        const radians = (player.angle - 90) * Math.PI / 180; // -90度オフセット（上向きを0度にする）
...
function updatePlayer(deltaTime) {
    ...
    if (aimDistance > 0.2) { // デッドゾーン
        // 照準方向にプレイヤーを向ける
        const aimAngle = Math.atan2(gameState.virtualInput.aim.y, gameState.virtualInput.aim.x) * 180 / Math.PI + 90;
